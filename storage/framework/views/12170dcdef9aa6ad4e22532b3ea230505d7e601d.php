

<?php $__env->startSection('content'); ?>

<!-- Hero Section end -->
<section class="hero-section">
  <div class="container">
    <div class="row">
      <div class="col-lg-12">
        <div class="hs-text">
          <h2>Let's Join Us in <span class="text-warning">Digital Marketing Class</span></h2>
          <p>Copy Writer. Content Creator. Videographer.</p>
          <a href="/beasiswa<?php echo e(Session::get('ref') != null ? '?ref='.Session::get('ref') : ''); ?>" class="site-btn sb-dark">Lihat Selengkapnya</a>
        </div>
      </div>
    </div>
  </div>
  <div class="hero-slider owl-carousel">
    <div class="hs-item set-bg" data-setbg="<?php echo e(asset('templates/loans2go/img/hero-slider/1.jpg')); ?>"></div>
    <div class="hs-item set-bg" data-setbg="<?php echo e(asset('templates/loans2go/img/hero-slider/2.jpg')); ?>"></div>
    <div class="hs-item set-bg" data-setbg="<?php echo e(asset('templates/loans2go/img/hero-slider/3.jpg')); ?>"></div>
  </div>
</section>
<!-- Hero Section end -->

<!-- Info Section -->
<section class="info-section spad">
  <div class="container">
    <div class="row">
      <div class="col-lg-5 text-center mb-2">
        <img src="<?php echo e(asset('assets/images/others/undraw_marketing_v0iu.svg')); ?>" style="max-width: 360px;" alt="">
      </div>
      <div class="col-lg-7">
        <div class="info-text">
          <h2 class="mb-4">Privat dan Kursus Internet Marketing</h2>
          <div class="accordion" id="accordionExample">
            <div class="card">
              <div class="card-header" id="headingOne">
                <h4 class="mb-0">
                  <button class="btn btn-link text-left font-weight-bold collapsed" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                    ANDA PEGAWAI, KARYAWAN, PENGUSAHA, ATAU SIAPAPUN YANG INGIN MENAMBAH PENGHASILAN?
                  </button>
                </h4>
              </div>
              <div id="collapseOne" class="collapse" aria-labelledby="headingOne" data-parent="#accordionExample">
                <div class="card-body">
                  Kami ada solusinya!! Belajar Online Marketing bersama kami di Campus Digital, di program Kursus Digital Marketing. Jadikan Bisnis Online sebagai sumber penghasilan Anda!!
                </div>
              </div>
            </div>
            <div class="card">
              <div class="card-header" id="headingTwo">
                <h4 class="mb-0">
                  <button class="btn btn-link text-left font-weight-bold collapsed" type="button" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                    ANDA INGIN BELAJAR BISNIS ONLINE TAPI TIDAK CUKUP WAKTU?
                  </button>
                </h4>
              </div>
              <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample">
                <div class="card-body">
                  Kami siapkan SOLUSInya!!! Kursus Digital Marketing yang kami adakan ini sudah kita siapkan untuk Anda yang sibuk. Pegawai, Karyawan, Mahasiswa, atau siapapun yang memiliki keterbatasan waktu. Kita desain sederhana tapi sangat efektif.
                </div>
              </div>
            </div>
            <div class="card">
              <div class="card-header" id="headingThree">
                <h4 class="mb-0">
                  <button class="btn btn-link text-left font-weight-bold collapsed" type="button" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                    ANDA INGIN USAHA SAMBILAN, TAPI TIDAK PUNYA CUKUP WAKTU? ATAU MALAH TIDAK CUKUP MODAL?
                  </button>
                </h4>
              </div>
              <div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
                <div class="card-body">
                  Campus Digital punya jawabannya!!! Bisnis Online. Bisa dikerjakan paruh waktu dan modal yang relatif terjangkau. Dan bisa dikerjakan siapapun dan di manapun.
                </div>
              </div>
            </div>
            <div class="card">
              <div class="card-header" id="headingFour">
                <h4 class="mb-0">
                  <button class="btn btn-link text-left font-weight-bold collapsed" type="button" data-toggle="collapse" data-target="#collapseFour" aria-expanded="false" aria-controls="collapseFour">
                    ANDA BINGUNG KEPADA SIAPA BELAJAR ONLINE MARKETING? APAKAH BELAJAR ONLINE MARKETING HARUS MAHAL?
                  </button>
                </h4>
              </div>
              <div id="collapseFour" class="collapse" aria-labelledby="headingFour" data-parent="#accordionExample">
                <div class="card-body">
                  Tidak usah BINGUNG!! Pengajar di Kursus Digital Marketing ini merupakan pengajar pilihan. Merupakan mentor dan supervisor terpilih dari Campus Digital. Pengajar kami bukan hanya mumpuni secara TEORI tapi juga bisnis onlinenya berjalan dan terbukti MENGHASILKAN.
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Info Section end -->

<!-- CTA Section end -->
<section class="cta-section set-bg" data-setbg="<?php echo e(asset('templates/loans2go/img/cta-bg.jpg')); ?>">
  <div class="container">
    <h2>Materi yang Diajarkan</h2>
    <h5>Materi Internet Marketing disampaikan oleh Praktisi Bisnis Online dan Trainer di Campus Digital.</h5>
    <a href="/beasiswa<?php echo e(Session::get('ref') != null ? '?ref='.Session::get('ref') : ''); ?>" class="site-btn sb-dark sb-big">Lihat Selengkapnya</a>
  </div>
</section>
<!-- CTA Section end -->

<!-- Why Section end -->
<section class="why-section spad">
  <div class="container">
<!--     <div class="text-center mb-5 pb-4">
      <h2>Why Choose us?</h2>
    </div> -->
    <div class="row">
      <div class="col-md-4">
        <div class="icon-box-item">
          <div class="ib-icon">
            <i class="flaticon-004-assistance"></i>
          </div>
          <div class="ib-text">
            <h5>Riset</h5>
            <p>Bagaimana cara untuk melakukan riset pasar dan produk.</p>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="icon-box-item">
          <div class="ib-icon">
            <i class="flaticon-024-laptop"></i>
          </div>
          <div class="ib-text">
            <h5>Social Media Marketing</h5>
            <p>Bagaimana menggunakan social media untuk aktivitas marketing.</p>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="icon-box-item">
          <div class="ib-icon">
            <i class="flaticon-032-placeholder"></i>
          </div>
          <div class="ib-text">
            <h5>Marketplace</h5>
            <p>Bagaimana berjualan menggunakan marketplace.</p>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="icon-box-item">
          <div class="ib-icon">
            <i class="flaticon-006-smartphone-2"></i>
          </div>
          <div class="ib-text">
            <h5>FB/IG Ads</h5>
            <p>Bagaimana beriklan di Facebook dan Instagram.</p>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="icon-box-item">
          <div class="ib-icon">
            <i class="flaticon-016-smartphone"></i>
          </div>
          <div class="ib-text">
            <h5>SEO Dasar</h5>
            <p>Bagaimana agar website nomor satu di halaman Search Engine.</p>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="icon-box-item">
          <div class="ib-icon">
            <i class="flaticon-037-responsive"></i>
          </div>
          <div class="ib-text">
            <h5>Desain Web</h5>
            <p>Bagaimana membuat website dan landing page untuk promosi.</p>
          </div>
        </div>
      </div>
    </div>
<!--     <div class="text-center pt-3">
      <a href="/register<?php echo e(Session::get('ref') != null ? '?ref='.Session::get('ref') : ''); ?>" class="site-btn sb-big">Daftar Sekarang!</a>
    </div> -->
  </div>
</section>
<!-- Why Section end -->

<!-- Help Section -->
<section class="help-section spad">
  <div class="container">
    <div class="feature-item">
      <div class="row">
        <div class="col-lg-6 text-center mb-2">
          <img src="<?php echo e(asset('assets/images/others/meeting.png')); ?>" style="max-width: 360px;" alt="">
        </div>
        <div class="col-lg-6">
          <div class="feature-text">
            <h2 class="text-white">Program Beasiswa Campus Digital</h2>
            <p class="text-light">Telah dibuka kembali Program Beasiswa Belajar Digital Marketing di Campus Digital. Gabung Segera!!!</p>
            <a href="/beasiswa<?php echo e(Session::get('ref') != null ? '?ref='.Session::get('ref') : ''); ?>" class="readmore">Detail dan Persyaratan <img src="<?php echo e(asset('templates/loans2go/img/arrow.png')); ?>" alt=""></a>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Help Section end -->

<!-- Feature Section -->
<section class="feature-section spad">
  <div class="container">
    <div class="text-center mb-5 pb-4">
      <h2>Mentor Internet Marketing</h2>
    </div>
    <div class="feature-item">
      <div class="row">
        <div class="col-lg-12">
          <div class="owl-carousel owl-theme" id="mentor">
            <div class="text-center">
              <img src="<?php echo e(asset('assets/images/others/faris.png')); ?>" class="mb-3 mx-auto" style="width: 150px!important;">
              <div class="h5">Faris Fanani</div>
              <p>Digital Business Strategy</p>
            </div>
            <div class="text-center">
              <img src="<?php echo e(asset('assets/images/others/danang.png')); ?>" class="mb-3 mx-auto" style="width: 150px!important;">
              <div class="h5">Danang Fata Akbar</div>
              <p>Social Media Expert</p>
            </div>
            <div class="text-center">
              <img src="<?php echo e(asset('assets/images/others/randy.png')); ?>" class="mb-3 mx-auto" style="width: 150px!important;">
              <div class="h5">Randy Rahman Hussein</div>
              <p>SEO Expert</p>
            </div>
            <div class="text-center">
              <img src="<?php echo e(asset('assets/images/others/gading.png')); ?>" class="mb-3 mx-auto" style="width: 150px!important;">
              <div class="h5">Gading Ariatama</div>
              <p>Content Creative</p>
            </div>
            <div class="text-center">
              <img src="<?php echo e(asset('assets/images/others/hamzah.png')); ?>" class="mb-3 mx-auto" style="width: 150px!important;">
              <div class="h5">Hamzah</div>
              <p>Copy Right & Publisher</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Feature Section end -->

<!-- CTA Section end -->
<section class="cta-section set-bg" data-setbg="<?php echo e(asset('templates/loans2go/img/score-bg.jpg')); ?>">
  <div class="container">
    <h2 class="text-white">Hubungi Kami</h2>
    <h5 style="color: #9e9fa5;">Daftar privat kursus internet marketing sekarang, atau hubungi kami untuk info lebih lanjut.</h5>
    <a href="https://wa.me/62816282135" class="site-btn sb-dark sb-big" target="_blank" style="background-color: #25D366; color: #333;"><i class="fa fa-whatsapp mr-2" style="font-size: 20px;"></i> Hubungi Kami via WhatsApp</a>
  </div>
</section>
<!-- CTA Section end -->

<!-- Feature Section -->
<section class="feature-section spad">
  <div class="container">
    <div class="text-center mb-5 pb-4">
      <h2>Testimoni</h2>
    </div>
    <div class="feature-item">
      <div class="row">
        <div class="col-lg-12">
          <div class="owl-carousel owl-theme" id="testimoni">
            <div class="card">
              <div class="card-body">
                <p>Pembelajaran di kelas ini sangat menyenangkan karena metode diajari step by step sehingga saya mudah sekali memahami.</p>
                <h5><i class="fa fa-user mr-2"></i> Adiyatma</h5>
              </div>      
            </div>
            <div class="card shadow">
              <div class="card-body">
                <p>Mentornya sabar mengajari kita-kita yang masih pemula sekali hingga bisa. Saya sarankan bila ingin bisa marketing online mengikuti kelas ini.</p>
                <h5><i class="fa fa-user mr-2"></i> Mila</h5>
              </div>      
            </div>
            <div class="card shadow">
              <div class="card-body">
                <p>Setelah mengikuti kelas ini saya bisa memahami A sampai Z bagaimana memasarkan produk online, diajari secara teknis dan komplit.</p>
                <h5><i class="fa fa-user mr-2"></i> Dafid</h5>
              </div>      
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Feature Section end -->

<!-- Score Section end -->
<!-- <section class="score-section text-white set-bg" data-setbg="<?php echo e(asset('templates/loans2go/img/score-bg.jpg')); ?>">
  <div class="container">
    <div class="row">
      <div class="col-xl-6 col-lg-8">
        <h2>Calculate my Score</h2>
        <h4>Check your credit reports as often as you want, it won't affect your scores.</h4>
        <a href="#" class="site-btn sb-big">show my score</a>
      </div>
    </div>
    <img src="<?php echo e(asset('templates/loans2go/img/hand.png')); ?>" alt="" class="hand-img">
  </div>
</section> -->
<!-- Score Section end -->

<?php $__env->stopSection(); ?>

<?php $__env->startSection('js-extra'); ?>

<script type="text/javascript">
  $(document).ready(function(){
    $('#mentor').owlCarousel({
      loop:true,
      autoplay:true,
      autoplayTimeout:2000,
      autoplayHoverPause:true,
      margin:10,
      responsiveClass:true,
      responsive:{
          0:{
              items:1,
              nav:true,
              loop:true,
          },
          576:{
              items:2,
              nav:false,
              loop:true,
          },
          768:{
              items:3,
              nav:false,
              loop:true,
          },
          992:{
              items:5,
              nav:true,
              loop:false,
              loop:true,
          }
      }
    });
  });

  $(document).ready(function(){
    $('#testimoni').owlCarousel({
      loop:true,
      autoplay:false,
      autoplayTimeout:2000,
      autoplayHoverPause:true,
      margin:10,
      responsiveClass:true,
      responsive:{
          0:{
              items:1,
              nav:true,
              loop:true,
          },
          576:{
              items:2,
              nav:false,
              loop:true,
          },
          768:{
              items:3,
              nav:false,
              loop:true,
          },
      }
    });
  });
</script>

<?php $__env->stopSection(); ?>

<?php $__env->startSection('css-extra'); ?>

<style type="text/css">
  .hero-section {height: calc(100vh - 180px);}
  .accordion .card {border-width: 0; border-bottom: 1px solid #e5e5e5!important;}
  .accordion .card .card-header {background-color: rgba(0,0,0,0); padding-left: .5rem; padding-right: .5rem;}
  .feature-text {padding-top: 0;}
</style>

<?php $__env->stopSection(); ?>
<?php echo $__env->make('template/guest/main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/vhosts/campusdigital.id/laravel-referral/resources/views/index.blade.php ENDPATH**/ ?>